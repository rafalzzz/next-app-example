...
export const PasswordField = <FormType extends object>({ formField }: FieldProps<FormType>) => {
  const [showPassword, setShowPassword] = useState(false);

  const { key, label, placeholder, register, error, isValueIncorrect, passwordFieldProps } =
    formField;
  ...
  return (
    <Styled.Label key={key}>
      ...
      {isValueIncorrect && <Styled.Error marginBottom={10}>{error}</Styled.Error>}
      {showHyperlink && (
